...
public class NOutputUtils<T> {
   ...
   */
  public Path[] listOutputFiles(FileSystem fs, Path outpath) throws IOException {
   ...
   */
  public void importEvaluations(FileSystem fs,
                                       Configuration conf,
                                       Path outpath,
                                       Collection<EvaluatedCandidateWritable<T>> evaluations) throws IOException {
	  
	TypeToken<EvaluatedCandidateWritable<T>> evalCandidateTypeToken = new TypeToken<EvaluatedCandidateWritable<T>>() {};
    SequenceFile.Sorter sorter = new SequenceFile.Sorter(fs, LongWritable.class, evalCandidateTypeToken.getRawType(), conf);
    ...
    for (EvaluatedCandidateWritable<T> value : new SequenceFileValueIterable<EvaluatedCandidateWritable<T>>(output, true, conf)) {
      
    	evaluations.add(new EvaluatedCandidateWritable<T>(value.getCandidate(), value.getFitness()));
    }
  }
